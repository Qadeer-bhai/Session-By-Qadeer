...
▬▬▬▬▬▬▬▬▬▬▬▬▬▬
©2024-2099 Qadeer-Khan_`
 await Pair_Code_By_Qadeer_Khan.sendMessage(Pair_Code_By_Qadeer_Khan.user.id,{text:QADEER_KHAN_TEXT},{quoted:session})
 

        await delay(100);
        await Pair_Code_By_Qadeer_Khan.ws.close();
        return await removeFile('./temp/'+id);
            } else if (connection === "close" && lastDisconnect && lastDisconnect.error && lastDisconnect.error.output.statusCode != 401) {
                    await delay(10000);
                    QADEER_KHAN_PAIR_CODE();
                }
            });
        } catch (err) {
